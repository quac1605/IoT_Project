    ...
    def get_frame(self):
        ...
        if(combine_value['lane_number'] == 2):
            ...
            if ((combine_value['angle'] >= 80) and auto_values['angle'] <= 100):
                ...
                auto_values['angle'] = auto_values['angle'] - 8
            elif (combine_value['angle'] >= 25 and (combine_value['angle']*1.32 - (auto_values['angle']) >= 5) and auto_values['angle'] <= 100):
                auto_values['angle'] = auto_values['angle'] + 4
            elif (combine_value['angle'] <= -25 and (combine_value['angle']*1.32 - (auto_values['angle']) <= -5) and auto_values['angle'] >= -100):
                auto_values['angle'] = auto_values['angle'] - 4
            elif (auto_values['angle'] <= 0):
            ...
            if ((check_side == 0) and (auto_values['angle'] > 35) or (auto_values['angle'] < -35)):
                auto_values['speed'] = 33
                auto_values['angle'] = -(auto_values['angle'] / abs(auto_values['angle']))*50
